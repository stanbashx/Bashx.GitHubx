...
if [[ -z "${GITHUBX_REF}" ]]; then
 echo 'No ref!' >&2; exit 1;
elif [[ ! "${GITHUBX_REF}" =~ ^[0-9a-f]{40}$ ]]; then
 echo 'Wrong ref!' >&2; exit 1;
fi
...
# https://docs.github.com/rest/git/tags

HTTP_CODE=$(curl -m 8 -w '%{http_code}' \
 "${GITHUBX_API}/repos/${GITHUBX_REP_OWNER}/${GITHUBX_REP_NAME}/git/tags/${GITHUBX_REF}" \
...
GITHUBX_TAG_SHA="$(yq -Mer -p=json -o=json '.sha' "${GITHUBX_DST}" 2>/dev/null)"
if [[ $? -ne 0 || "${GITHUBX_TAG_SHA}" != "${GITHUBX_REF}" ]]; then
